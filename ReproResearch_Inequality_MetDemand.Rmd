...
# get sub-regional classification from DHS
url<-("http://api.dhsprogram.com/rest/dhs/countries?f=json")
jsondata<-fromJSON(url) 
dtaregion<-data.table(jsondata$Data)
    names(dtaregion)
dtaregion<-select(dtaregion, CountryName, SubregionName)
    dim(dtaregion)
    names(dtaregion)
    table(dtaregion$SubregionName)
dtaregion<- dtaregion %>% rename (country	=	CountryName)
dtaregion<- dtaregion %>% rename (region2	=	SubregionName)

dta<-full_join(dtaregion, dta, by = "country")
```
